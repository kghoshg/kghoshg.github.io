...
def run_Newton_ADMM(rank, size,Lambda,CG_tol,CG_maxit,data_path,Newton_maxit,LS_maxit):
	...
	## Slave: Starts Newton-ADMM routine
		...
		for epoch in range(Newton_maxit):
			...
			CG_iter,local_dir,g,norm_g,rel_residual=NewtonMR(x,y,W_cur,Lambda,CG_tol,CG_maxit,cols,num_classes,W_cur.grad,num_nodes,np.asscalar(rho_cur.data.cpu().numpy()),Variable(  torch.DoubleTensor(cols, num_classes).zero_().cuda(),volatile=True ))
